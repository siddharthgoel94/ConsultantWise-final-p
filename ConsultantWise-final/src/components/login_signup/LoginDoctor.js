...
const senddata = async (e) => {
  ...
  console.log(logdata);
  try {
      const res = await fetch("http://localhost:3001/loginDoctor", {
          method: "POST",
          headers: {
    ...
    <div className="md:w-1/3 max-w-sm space-y-7">
      ...
      <div className="mt-4 font-bold  text-red-900 text-center md:text-center text-lg">
        Welcome To Consultant's Login Portal
      </div>
      ...
      <div className="mt-4 font-semibold  text-slate-700 text-center md:text-left text-md">
        <NavLink to="/login" className="text-red-700 hover:underline hover:underline-offset-4 " href="#">Login As a Client</NavLink>
      </div>
      <ToastContainer/>
    </div>
  </section>
